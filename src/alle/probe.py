...
def probe_channel(port: int, timeout: int = 10) -> dict:
    ...
    times out, or yields an empty / non-IP / non-public body. A clear error
    naming the sources tried is recorded only once *all* of them fail. Each
    request is independently bounded by ``timeout``.
    """
    opener = _opener(port)
    attempted: list[str] = []
